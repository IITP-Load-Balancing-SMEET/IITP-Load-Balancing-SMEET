...
class SCENARIO:
    ...
    def lidar_callback(self, lidar):
        ...
        lidar.save_to_disk(os.path.join(self.dataset_path),
                           '/liDAR/', str(self.junction_id), file_name)
    ...
    def set_traffic_manger(self, synchronous=True):
        ...
        self.traffic_manager.ignore_lights_percentage(
            self.aggressive_car_percenatge)
        ...
        platoon_spawn_points = {'merge1': self.map.get_waypoint_xodr(road_id=38, lane_id=-4, s=160.0).transform,
                                'merge2': self.map.get_waypoint_xodr(road_id=40, lane_id=6, s=-180.0).transform,
                                'split1': self.map.get_waypoint_xodr(road_id=39, lane_id=-4, s=103.0).transform,
                                'split2': self.map.get_waypoint_xodr(road_id=39, lane_id=6, s=27.0).transform}

        for _ in range(self.num_nv):
            spawn_point = platoon_spawn_points[self.type]
            vehicle_bp = random.choice(
                self.bp.filter('vehicle.bmw.grandtourer'))
            temp_vehilce = self.world.try_spawn_actor(vehicle_bp, spawn_point)

            if temp_vehicle is not None:
                spawn_point.location.x -= 5.0
                self.nv_list.append(temp_vehicle)
        ...
        [actor.set_autopilot(True) for actor in self.nv_list[::-1]]
    ...
    def junction_lidar_spawn(self):
        ...
        lidar_bp.set_attribute('channels', int(
            self.sensor_config['points_per_second']))
        lidar_bp.set_attribute('channels', int(
            self.sensor_config['rotation_frequency']))
