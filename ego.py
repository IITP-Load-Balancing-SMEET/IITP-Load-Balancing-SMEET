...
class EGO(SCENARIO):
    ...
    def spawn_camera(self):
        ...
        cam2_transform = carla.Transform(carla.Location(
            x=1.5, y=-2.0, z=2.0), carla.Rotation(yaw=-30.0))
        cam3_transform = carla.Transform(carla.Location(
            x=1.5, y=2.0, z=2.0), carla.Rotation(yaw=30.0))
    ...
    def img_callback(self):
        ...
        if (img_front is not None) and (img_left is not None) and (img_right is not None) and (depth_img is not None):
            ...
            img_left = np.reshape(np.copy(img_left.raw_data),
            ...
            if self.save:
                cv2.imwrite(self.dataset_path +
                            f"/images/depth/{int(d_timestamp)}.png", depth_img)
                cv2.imwrite(
                    self.dataset_path + f"/images/rgb/{int(rgb_timestamp):010d}.png", rgb_img)
    ...
    def gnss_callback(self, gnss):
        ...
        x = radius * (lon - self.INIT_LON) * self.drad * \
    ...
    def Near_vehicle(self):
        ...
        for i in range(len(self.actor_list)-1):
            ...
            distance = math.sqrt(math.pow(
                (ego_location.x-temp_location.x, 2)+(ego_location.y-temp_location.y, 2)))
            if distance < 50:
    ...
    def update_view(self):
        try:
            ...
            self.spectator.set_transform(carla.Transform(
                transform.location + carla.Location(x=-5.0, y=0.0, z=2.0)))
    ...
    def cleanup(self):  # __del__ is not working
        ...
        destroy_commands2 = [carla.command.DestroyActor(
            actor.id) for actor in self.nv_list]
        destroy_commands3 = [carla.command.DestroyActor(
            actor.id) for actor in self.junction_list]
